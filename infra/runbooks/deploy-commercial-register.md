# Deploy Finite Business

This runbook deploys the internal Finite Business application to the same
dedicated VM as Grafana. Its first module is the commercial relationship
register. It does not seed customer data. The synthetic NED fixture is a test
...
- `scripts/finite-status --json`, staged on `finite-monitoring-1-81926`, is
  ...
  the Finite Business units, four exact image digests, backups, and restore
  health.
- `/etc/finite/commercial-register/borg.env` and its private SSH key have been
  ...
  Finite Business commercial module and reachable with append-only credentials
...
- The Namecheap `finite.computer` zone can receive an `A` record for `business`.
...
## Publish `business.finite.computer`

1. Create the Namecheap DNS record `A business 152.236.5.27`. Keep the initial
   TTL low enough to correct a mistake quickly.
2. Wait until `dig +short A business.finite.computer` returns `152.236.5.27`
...
at the host. Prometheus then probes `https://business.finite.computer/healthz`.

## Install the versioned application

1. Sign in at `https://business.finite.computer` with the already-claimed admin
   and create one narrowly scoped API key for deploying the versioned
   application. Keep the value outside git and shell history.
2. Configure the Twenty CLI remote, preview the application plan, then apply it:

   ```console
   cd commercial-register
   corepack yarn twenty remote:add --as finite-production --url https://business.finite.computer --api-key "$FINITE_COMMERCIAL_TWENTY_API_KEY"
...
```console
curl --fail https://business.finite.computer/healthz
ssh -T ubuntu@152.236.5.27 \
...
1. Remove `business.finite.computer` from the Caddy config or temporarily point
   DNS away if the public origin must be fenced.
